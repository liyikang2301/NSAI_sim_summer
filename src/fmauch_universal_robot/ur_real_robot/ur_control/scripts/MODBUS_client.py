...
#本代码集成了PLC通信和机械臂控制的系统，提供了对机械臂角度、转速和位置等方面的精细控制，代码通过ROS与PLC进行交互，并通过MODBUS协议发送控制命令。

class MODBUS_control(): #封装与PLC通信并控制机械臂的各项功能
    # plc是103，先连上
    def __init__(self, host = "192.168.56.103", port = 502): #初始化类时，连接到指定的PLC主机和端口
        self.host = host
        self.port = port

        # 这里记录的是不同的角度对应的plc中的数值，输入这些数值就代表了希望变化的角度
        self.sleeve_ang2reg = {"45.0":16948,"-315.0":16948, #保存角度与寄存器值之间的映射关系
    ...
    # 设置开始转
    def set_sleeve_rotation(self): #启动旋转动作
        register = 152
        value = 256
    ...
    def set_effector_stop(self): #停止执行器
        register = 154
        value = 256
        self.modclient.setOutput(register,value,0)

    def read_effector_speed(self): #读取当前执行器的速度
        register = 158
        value = 256
        self.modclient.setOutput(register,value,0)
        print("speed:",self.modclient.readRegisters(103,2))

    def read_effector_state(self): #读取当前执行器状态
        register = 160
        value = 256
        self.modclient.setOutput(register,value,0)
...
if __name__ == '__main__':
    rospy.init_node('plc', anonymous=True) #初始化一个ROS节点
    plc_control=MODBUS_control() #创建MODBUS_control实例
    ...
    plc_control.set_effector_star_pos(4000) #末端执行器将以4000的速度正转2秒钟，然后停止
    time.sleep(2)
