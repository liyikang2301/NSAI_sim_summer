...
#本来带用于从3D点云数据中估计一个平面，并计算物体相对相机的位置

def _check_data_atleast_3Dpoints(data):  # pre-check of the data
    if data.ndim < 2 or data.shape[1] != 3:
        raise ValueError('Input data must be 3D points!')
    #检查输入数据是否为3D点云数据。如果数据维度不符合要求，就抛出异常。


class PlaneModel(): #用于拟合一个平面模型
    def __init__(self):
        self.params = None

    def estimate(self, data): #通过SVD（奇异值分解）估计平面模型。它计算点云的质心，并基于质心平移数据，然后通过SVD计算法向量。如果点云点数不足或点都在同一条线上，模型估计失败。
    ...
    def residuals(self, data, params=None): #计算每个点到平面的距离，即残差
...
def cal_ransac_plane(data, min_samples=3, residual_threshold=0.001,
                     ...
                     stop_probability=1): #使用RANSAC（随机抽样一致性算法）来拟合一个平面
    """
...
def depth_filter(point_cloud): #过滤点云中深度值为0的点，因为这些点通常是无效数据。
    """
...
def clamp(x,minn,maxn): #确保坐标不会超出图像的边界。
    return max(min(maxn, x), minn)

def generate_selected_points(tl_x, tl_y, br_x, br_y, all_info): #根据输入的矩形区域，选择图像中的ROI（感兴趣区域），生成对应的3D点云。
    delta=10
    tl_x=tl_x-delta
    tl_y=tl_y-delta
    br_x=br_x+delta
...
def normalize(v): #将向量归一化，使得其长度为1。
...
def rectify_vector(v): #矫正向量的方向，使其与标准方向一致。
    if v[0, -1] < 0:
        return -v
    else:
        return v

def not_same_pose(v): #检查向量是否与标准方向（[0, 0, 1]）相同。
    if np.allclose(v[0],[0,0,1],rtol=1e-04,atol=1e-08):
        return False
    else:
        return True

def cal_bolt_plane(tl_x, tl_y, br_x, br_y, all_info): #这个函数用于计算螺栓的平面，并且输出螺栓相对于相机的坐标。
