...
#本代码创建一个基于VGG神经网络的图像分类服务，通过Socket通信，能够接受来自客户端的图像数据，使用预训练的模型进行分类，然后将结果返回给客户端。

def img_primitive(img): #处理图像，进行前向传播，并获得预测结果
    ...
    align_weights_path='./VGG_summer.pth'
    ...
    obstacle_weights_path='./VGG_summer.pthVGG_summer.pth'
